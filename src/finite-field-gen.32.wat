...
(module 
  ...
  (func $multiply (param $xy i32) (param $x i32) (param $y i32)
    (local $tmp i64) (local $carry2 i64) (local $m i64) (local $carry1 i64)
    ...
    (loop 
      ...
      i64.add
      (i64.const 32) i64.shr_u (local.set $carry2)
      ...
      (local.get $xi)
      (local.get $y01)
      i64.mul (local.get $carry1) i64.add i64.add
      (local.set $tmp)
      ;; A = tmp >> w
      (local.set $carry1 (i64.shr_u (local.get $tmp) (i64.const 32)))
      ;; tmp = (tmp & 0xffffffffn) + m * p[j] + C
      (i64.and (local.get $tmp) (i64.const 0xffffffff))
      (i64.mul (local.get $m) (i64.const 0xb9feffff))
      (local.get $carry2) i64.add i64.add
      (local.set $tmp)
      ;; (C, t[j - 1]) = tmp
      (local.set $t00 (i64.and (local.get $tmp) (i64.const 0xffffffff)))
      (local.set $carry2 (i64.shr_u (local.get $tmp) (i64.const 32)))
      ...
      ;; t[11] = A + C
      (local.set $t11 (i64.add (local.get $carry1) (local.get $carry2)))
