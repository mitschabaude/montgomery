...
async function benchmarkMsm(params: CurveParams, n: number, nThreads?: number) {
  ...
  await Parallel.msmProjective(scalarPtr, pointPtr, 1 << 15);
  ...
  for (let i = 0; i < 15; i++) {
    let [scalarPtr] = await Parallel.randomScalars(N);
    tic();
    await Parallel.msmProjective(scalarPtr, pointPtr, 1 << n);
    let time = toc();
    if (i > 4) times.push(time);
  }
  [scalarPtr] = await Parallel.randomScalars(N);
  tic();
  let { log } = await Parallel.msmProjective(scalarPtr, pointPtr, 1 << n);
  let t = toc();
